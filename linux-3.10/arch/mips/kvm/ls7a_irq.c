	...
	if (offset & (len -1 )) {
		...
		data = *(uint64_t *)val;
				...
				ls7a_update_read_page_long(s,offset,state->htmsi_en);
                                break;
			case LS7A_IOAPIC_AUTO_CTRL0_OFFSET:
			case LS7A_IOAPIC_AUTO_CTRL1_OFFSET:
				break;
                        default:
...
static int kvm_ls7a_ioapic_alias_read(struct kvm_io_device *this,
				gpa_t addr, int len, void *val)
{
	struct loongson_kvm_7a_ioapic *s;
	unsigned long flags;

	s = container_of(this, struct loongson_kvm_7a_ioapic,
						ls7a_ioapic_alias);
	ls7a_ioapic_lock(s->kvm->arch.v_ioapic, &flags);
...
static int kvm_ls7a_ioapic_alias_write(struct kvm_io_device *this,
				gpa_t addr, int len, const void *val)
{
	struct loongson_kvm_7a_ioapic *s;
	unsigned long flags;
